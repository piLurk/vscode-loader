...
import { Environment, globalVar, _commonjsGlobal } from "./env";
import { LoaderEventType, ILoaderEventRecorder } from "./loaderEvents";
import { Utilities } from "./utils";
import {
  IDefineFunc,
  INodeCachedDataConfiguration,
  IRequireFunc,
  Configuration,
} from "./configuration";
...
const isCrossOriginFn = (scriptSrc: string) => {
  return (
  ...
  public async load(
    ...
    if (/^node\|/.test(scriptSrc)) {
      ...
      let nodeRequire = ensureRecordedNodeRequire(
        ...
        opts.nodeRequire || globalVar.nodeRequire
      );
      ...
      try {
        ...
        errorback(
          new Error(`${err.message} &-1 ${scriptSrc} & ${pieces.join(",")}`)
        );
      ...
      if (text) {
        ...
        window.__icubeResolveModule__ = function (payload) {
          if (payload?.type === "__icube_module_loaded__") {
            const { loaderID, origin } = payload;
            if (
              origin === originScriptSrc &&
              loaderID === globalVar.getUniqueID()
            ) {
              callback();
            }
          }
    ...
    if (/^node\|/.test(scriptSrc)) {
      ...
      try {
        ...
        const newErr = new Error(
          `${err.message} &-2 ${scriptSrc} & ${pieces.join(",")}`
        );
        errorback(newErr);
        return;
      }
  ...
  public load(
    ...
    if (/^node\|/.test(scriptSrc)) {
      ...
      try {
        ...
        const newErr = new Error(
          `${err.message} &-3  ${scriptSrc} & ${pieces.join(",")}`
        );
        errorback(newErr);
